          ...
          {Object.entries(grouped).map(([cat, catQs]) => (
            <div key={cat}>
              <h3 className="text-xs font-semibold text-violet-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                ...
                {cat}
                <span className="text-[var(--text-muted)] font-normal lowercase">({catQs.length})</span>
              </h3>
                  ...
                  <div key={q.id} className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg p-4 hover:border-[#3a4a65] transition">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0 space-y-3">
                        {canEdit ? (
                          <input className="w-full bg-transparent text-[var(--text)] text-sm font-medium focus:outline-none border-b border-transparent focus:border-blue-500/50 pb-1"
