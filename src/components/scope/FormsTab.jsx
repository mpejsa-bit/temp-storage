...
export default function FormsTab({ data, canEdit, onSave }) {
  ...
  const FormFieldBuilder = ({ form }) => {
    ...
    return (
      <div className="mt-4 border-t border-[#2a3a55]/30 pt-4">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-xs font-semibold text-cyan-400 uppercase tracking-wider">
            Form Fields ({fields.length})
            <span className="text-[#4a5568] font-normal ml-2">
        ...
        {fields.length === 0 ? (
          <p className="text-[10px] text-[#4a5568] italic">No fields defined. Click Add Field to start building.</p>
        ) : (
          <div className="bg-[#0a0e17] border border-[#2a3a55] rounded-lg overflow-auto max-h-[400px]">
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-[#1e3a5f]/80">
                <tr>
                  <th className="px-2 py-2 text-left text-[#94a3b8] font-semibold w-8">#</th>
                  <th className="px-2 py-2 text-left text-[#94a3b8] font-semibold">Field Name</th>
                  <th className="px-2 py-2 text-left text-[#94a3b8] font-semibold">Description</th>
                  <th className="px-2 py-2 text-left text-[#94a3b8] font-semibold w-28">Field Type</th>
                  <th className="px-2 py-2 text-left text-[#94a3b8] font-semibold">Options</th>
                  <th className="px-2 py-2 text-left text-[#94a3b8] font-semibold w-20">Source</th>
                  <th className="px-2 py-2 text-left text-[#94a3b8] font-semibold">Example</th>
                  {canEdit && <th className="px-2 py-2 w-8"></th>}
                </tr>
              </thead>
              <tbody>
                {fields.map((f, i) => (
                  <tr key={i} className="border-t border-[#2a3a55]/20 hover:bg-[#111827]">
                    <td className="px-2 py-1.5 text-[#4a5568]">{i + 1}</td>
                    <td className="px-2 py-1.5">
                      <input className="w-full bg-transparent text-white text-xs focus:outline-none" value={f.name || ""} onChange={e => updateField(i, "name", e.target.value)} disabled={!canEdit} placeholder="Field name"/>
                    </td>
                    <td className="px-2 py-1.5">
                      <input className="w-full bg-transparent text-[#94a3b8] text-xs focus:outline-none" value={f.description || ""} onChange={e => updateField(i, "description", e.target.value)} disabled={!canEdit} placeholder="Description"/>
                    </td>
                    <td className="px-2 py-1.5">
                      <select className="bg-[#111827] text-white text-xs border border-[#2a3a55] rounded px-1 py-0.5 focus:outline-none focus:border-blue-500" value={f.field_type || "Text"} onChange={e => updateField(i, "field_type", e.target.value)} disabled={!canEdit}>
                        {FIELD_TYPES.map(ft => <option key={ft} value={ft}>{ft}</option>)}
                      </select>
                    </td>
                    <td className="px-2 py-1.5">
                      <input className="w-full bg-transparent text-[#94a3b8] text-xs focus:outline-none" value={f.options || ""} onChange={e => updateField(i, "options", e.target.value)} disabled={!canEdit} placeholder="MC options..."/>
                    </td>
                    <td className="px-2 py-1.5">
                      <input className="w-full bg-transparent text-[#94a3b8] text-xs focus:outline-none" value={f.source || ""} onChange={e => updateField(i, "source", e.target.value)} disabled={!canEdit} placeholder="Source"/>
                    </td>
                    <td className="px-2 py-1.5">
                      <input className="w-full bg-transparent text-[#94a3b8] text-xs focus:outline-none" value={f.example || ""} onChange={e => updateField(i, "example", e.target.value)} disabled={!canEdit} placeholder="Example"/>
                    </td>
                    {canEdit && (
                      <td className="px-2 py-1.5">
    ...
    <div className="space-y-6">
      ...
      {forms.length === 0 ? (
        <div className="text-center py-16 bg-[#111827] border border-[#2a3a55] rounded-xl">
          ...
          <p className="text-[#4a5568] text-sm">No forms defined yet.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {forms.map((form, i) => (
            <div key={form.id} className="bg-[#111827] border border-[#2a3a55] rounded-lg overflow-hidden hover:border-[#3a4a65] transition">
              {/* Form header row */}
              <div className="p-4 cursor-pointer" onClick={() => toggleExpand(form.id)}>
                <div className="flex items-center gap-3">
                  {expanded === form.id ? <ChevronDown className="w-4 h-4 text-blue-400 flex-shrink-0"/> : <ChevronRight className="w-4 h-4 text-[#4a5568] flex-shrink-0"/>}
                  <span className="text-[#4a5568] text-xs font-mono w-8">{form.form_number || i + 1}</span>
                  {canEdit ? (
                    <input className="flex-1 bg-transparent text-white text-sm font-medium focus:outline-none" value={form.form_name || ""} onClick={e => e.stopPropagation()} onChange={e => updateForm(form, "form_name", e.target.value)} placeholder="Form name..."/>
                  ) : (
                    <span className="text-white text-sm font-medium">{form.form_name || "Untitled"}</span>
                  )}
              ...
              {/* Expanded detail */}
              {expanded === form.id && (
                <div className="px-4 pb-4 border-t border-[#2a3a55]/30">
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Purpose / Current Use</label>
                      <textarea className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 resize-y disabled:opacity-50"
                        rows={2} value={form.purpose || ""} onChange={e => updateForm(form, "purpose", e.target.value)} disabled={!canEdit}/>
                    </div>
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Used in Workflow?</label>
                      <select className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 disabled:opacity-50"
                    ...
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Driver or Dispatch Initiated</label>
                      <select className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 disabled:opacity-50"
                    ...
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Driver Response Expected?</label>
                      <select className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 disabled:opacity-50"
                    ...
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Form Category</label>
                      <select className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 disabled:opacity-50"
                    ...
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Form Platform</label>
                      <select className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 disabled:opacity-50"
                    ...
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Decision Tree Logic?</label>
                      <select className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 disabled:opacity-50"
                    ...
                    <div>
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Stored Procedures?</label>
                      <select className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 disabled:opacity-50"
                  ...
                  {form.stored_procedures === 1 && (
                    <div className="mt-3">
                      <label className="text-[10px] text-[#64748b] uppercase tracking-wider">Describe Stored Procedures</label>
                      <textarea className="w-full mt-1 px-2 py-1.5 bg-[#0a0e17] border border-[#2a3a55] rounded text-white text-xs focus:outline-none focus:border-blue-500 resize-y disabled:opacity-50"
